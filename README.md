# rvlibs

[![CI](https://github.com/reasvyn/rvlibs/actions/workflows/ci.yml/badge.svg)](https://github.com/reasvyn/rvlibs/actions)
[![License](https://img.shields.io/crates/l/rvlibs.svg)](LICENSE)

**Modular · Atomic · Composite**

## Crates

| Crate | Description |
|---|---|
| [**rvmath**](crates/rvmath/) | Comprehensive, lightweight, type-safe mathematics library |
| [**rvtest**](crates/rvtest/) | Next Level Testing Library — BDD specs, property tests, coverage |
| [**rvtest-macros**](crates/rvtest-macros/) | Proc-macro API for rvtest |
| [**cargo-rvtest**](crates/cargo-rvtest/) | CLI binary for rvtest |

## License

Dual-licensed under [MIT](LICENSE) or [Apache-2.0](https://www.apache.org/licenses/LICENSE-2.0).
