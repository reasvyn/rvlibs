...
> **A Next Level Testing Library for Rust**
...
### 6. Assertion Macros with Diff Output
...
fn structured_diffs() {
    ...
    rvtest::assert_delta!(1.0, 1.0, 0.001);
}
```
...
### 8. Proc-Macro API (optional)

Enable via `Cargo.toml`:
```toml
rvtest = { version = "0.3.0", features = ["macros"] }
```

Then use attribute macros instead of the builder API:
```rust
use rvtest::*;
...
cargo rvtest -F github      # GitHub Actions annotations
...
# Fast mode (fast linker + no debug info)
cargo rvtest --fast
...
# Daemon mode (persistent compile daemon)
cargo rvtest --daemon
